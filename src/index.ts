...
const server = new McpServer(
  ...
  },
  {
    instructions: [
      ...
      "- `opencode_fire_async` — OPENCLAW SPECIAL: async task with automatic webhook callback. Use when OpenClaw initiates tasks and needs notification upon completion. Task completes in background and OpenClaw receives webhook automatically.",
      ...
      "- `opencode_async_task_status` — check status of async task created with `opencode_fire_async` (read-only)",
      "- `opencode_async_tasks_list` — list all async tasks (read-only)",
      "",
      ...
      "```",
      "",
      "### OpenClaw async workflow (automatic callback on completion):",
      "```",
      "// OpenClaw initiates a long-running task and gets notified automatically",
      'opencode_fire_async({',
      '  prompt: "Refactor the entire codebase to TypeScript...",',
      '  callbackUrl: "https://openclaw.example.com/webhook/opencode-completion",',
      '  providerID: "anthropic",',
      '  modelID: "claude-opus-4-6"',
      '})',
      "// → Returns immediately with taskId and sessionId",
      "// → OpenCode works in the background",
      "// → When task completes, OpenClaw receives webhook callback automatically",
      "",
      "// Check task status anytime (optional - webhook is automatic):",
      'opencode_async_task_status({taskId: "task_xxx"})',
      "```",
      "",
